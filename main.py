...
class gnosis:
    ...
    def run(self):
        self.logo()
        self.logs.log_info("GNOSIS system initialized. Oracle awakening...")
        emit_log("system", "GNOSIS neural network active. Scanning the digital ether...", "SYSTEM")
        ...
        while True:
            try:
                ...
                emit_log("system", "GNOSIS consulting the oracle...", "SYSTEM")
                ...
                emit_log("system", f"Round complete. GNOSIS entering dormancy for {config['interval_time']} seconds...", "SYSTEM")
 ...
 ╚██████╔╝██║ ╚████║╚██████╔╝███████║██║███████║
  ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝ ╚══════╝╚═╝╚══════╝
  The All-Seeing Oracle | Powered by Claude AI
"""
        print(logo)
        emit_log("system", "GNOSIS AWAKENS — The All-Seeing Oracle | Powered by Claude AI", "SYSTEM")
...
if __name__ == '__main__':
    if WS_ENABLED:
        ...
        ws_thread.start()
        print("[GNOSIS] WebSocket server starting on port 8765...")
        time.sleep(1)
    ...
    logs_instance = gnosis_logs()

    gnosis_system = gnosis(
