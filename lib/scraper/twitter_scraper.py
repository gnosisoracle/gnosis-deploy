...
class Twitter_Scraper:
    ...
    def _get_driver(
        ...
        # Auto-detect Chrome binary (required on Render)
        import subprocess as _sp
        _chrome = os.environ.get("CHROME_BINARY_PATH","")
        if not _chrome or not os.path.exists(_chrome):
            for _c in ["/usr/bin/google-chrome-stable","/usr/bin/google-chrome",
                       "/opt/google/chrome/google-chrome","/opt/google/chrome/chrome",
                       "/usr/bin/chromium","/usr/bin/chromium-browser"]:
                if os.path.exists(_c): _chrome=_c; break
        if not _chrome:
            try:
                for _name in ["google-chrome-stable","google-chrome","chromium"]:
                    _r=_sp.run(["which",_name],capture_output=True,text=True)
                    if _r.returncode==0: _chrome=_r.stdout.strip(); break
            except: pass
        if _chrome:
            browser_option.binary_location = _chrome
            print(f"Scraper Chrome binary: {_chrome}")
        else:
            print("Scraper: no Chrome binary found, using selenium default")
